...
// "Our Services" href="#" — clicking never navigates, only opens dropdown
const NAV_LINKS: NavLinkItem[] = [
    { label: "Home", href: "/" },
    { label: "About Us", href: "/about-us" },
...
function MegaDropdown({ visible, onMouseEnter, onMouseLeave, onClose }: MegaDropdownProps) {
    return (
        <div
            onMouseEnter={onMouseEnter}
            onMouseLeave={onMouseLeave}
                ...
                {SERVICES.map((svc) => (
                    ...
                    >
                        ...
                        <div
                            className="mt-0.5 w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-all duration-200"
                            style={{
                                background: "rgba(139,26,140,0.18)",
...
function MobileServicesAccordion({ open, onLinkClick }: MobileServicesAccordionProps) {
    return (
        <div
            className={`overflow-hidden transition-all duration-300 ${open ? "max-h-[680px]" : "max-h-0"}`}
        >
            <div
                className="ml-4 pl-4 pt-1 pb-3 flex flex-col gap-0.5"
                style={{ borderLeft: "2px solid rgba(139,26,140,0.25)" }}
            >
                {SERVICES.map((svc) => (
                    <NavLink
                        key={svc.label}
...
export default function Navbar() {
    ...
    // Instant close — called when any link inside the dropdown is clicked
                ...
                <nav className="hidden lg:flex items-center gap-1" aria-label="Main navigation">
                    {NAV_LINKS.map((link) => {
                        ...
                        return (
                            ...
                            >
                                {isServices ? (
                                    // ── "Our Services": button, never navigates,
                                    //    click toggles dropdown, hover handled by parent div ──
                                    <button
                                        onClick={() => setDropdownOpen((o) => !o)}
                                        className="relative flex items-center gap-1.5 px-4 py-2 rounded-md transition-colors duration-200 whitespace-nowrap cursor-pointer"
                                        style={{
                                            ...linkStyle,
                                            color: dropdownOpen
                    ...
                    >
                        <NavLink
                            ...
                            style={{
                                ...
                                letterSpacing: "0.1em",
                                fontSize: "0.9rem",
                                boxShadow: "0 4px 16px rgba(0,165,80,0.30)",
                            }}
                            onClick={() => setMobileOpen(false)}
                        >
                            Get a Free Consultation
